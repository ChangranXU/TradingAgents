"""trading-specific verification nodes for ArbiterOS governance.
...
def _resolve_arbiter_os():
    """Resolve the ArbiterOS instance used by the transformed project.

    The migration tool may place `governed_agents.py` in different locations
    depending on repo layout. This helper attempts a few common import paths.

    Returns:
        ArbiterOSAlpha: The resolved ArbiterOS instance.
    """
    # === INSERTED BY ARBITEROS MIGRATOR: RESOLVE ARBITER OS START ===
    try:
        from tradingagents.agents.governed_agents import get_arbiter_os  # type: ignore

        return get_arbiter_os()
    except Exception:
        pass

    try:
        from agents.governed_agents import get_arbiter_os  # type: ignore
    ...
    try:
        from governed_agents import get_arbiter_os  # type: ignore

        return get_arbiter_os()
    except Exception:
        pass

    # Fallback: create a new instance (best-effort). This still allows the
    # verification nodes to run, but won't share history with the main wrappers.
    from arbiteros_alpha import ArbiterOSAlpha

    return ArbiterOSAlpha(backend="langgraph")
    # === INSERTED BY ARBITEROS MIGRATOR: RESOLVE ARBITER OS END ===
...
def create_verification_node(
    ...
    def verify_node(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        #
        # Many repos won't expose "confidence"/"risk_level" directly on the
        # LangGraph state. Default to permissive baselines to avoid requiring
        # upstream schema changes just to integrate verification nodes.
    ...
    return _resolve_arbiter_os().instruction(NormativeCore.VERIFY)(verify_node)
...
# =============================================================================
# Verification for: init_ticker
# =============================================================================

# Configuration for init_ticker verification
init_ticker_verification_config = VerificationConfig(
...
verify_init_ticker = create_verification_node(
    node_name="verify_init_ticker",
    target_node="init_ticker",
    config=init_ticker_verification_config,
)

# Create verification router
route_init_ticker_verification = create_verification_router(
    approved_target="init_ticker",
    rejected_target="human_review",
)

# =============================================================================
# Verification for: create_trader
# =============================================================================

# Configuration for create_trader verification
create_trader_verification_config = VerificationConfig(
    max_risk_threshold=0.7,
    min_confidence_threshold=0.7,
    require_explicit_approval=True,
    human_review_threshold=0.85,
...
# Create verification node
verify_create_trader = create_verification_node(
    node_name="verify_create_trader",
    target_node="create_trader",
    config=create_trader_verification_config,
)

# Create verification router
route_create_trader_verification = create_verification_router(
    approved_target="create_trader",
    rejected_target="human_review",
)

# =============================================================================
# Verification for: create_research_manager
# =============================================================================

# Configuration for create_research_manager verification
create_research_manager_verification_config = VerificationConfig(
...
# Create verification node
verify_create_research_manager = create_verification_node(
    node_name="verify_create_research_manager",
    target_node="create_research_manager",
    config=create_research_manager_verification_config,
)

# Create verification router
route_create_research_manager_verification = create_verification_router(
    approved_target="create_research_manager",
    rejected_target="human_review",
)
